...
export function deprecatedFieldsHaveReason(opts?: { severity?: 'error' | 'warn' }): PolicyRule {
  const severity = opts?.severity ?? 'warn';

  return {
    name: 'deprecated-fields-have-reason',
    description: 'Fields with @deprecated directive must include a non-empty reason',
    severity,
    check(schema: GraphQLSchema): Violation[] {
      ...
      for (const [typeName, type] of Object.entries(typeMap)) {
        ...
        for (const [fieldName, field] of Object.entries(fields)) {
          if (!field.deprecationReason && field.deprecationReason !== undefined) {
          ...
          if (field.astNode) {
            ...
            if (deprecatedDirective) {
              ...
              if (!reasonArg || ('value' in reasonArg.value && !(reasonArg.value as { value: string }).value?.trim())) {
